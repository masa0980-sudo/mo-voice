...
- **誤認識を覚える** — 間違いを一度直すと、次から自動で同じ修正が適用されます。
  1〜2文字違いの言い間違いの揺れ（例: 「クロードコード」を直した後の「プロドコード」）も拾います
- **自信のない箇所を色で示す** — 認識が怪しい部分を黄色でハイライトします
- **長い発話でも待たされにくい（任意）** — 録音しながら先に認識を進める「逐次認識」を
  オンにすると、3〜5分の口述でも停止後の待ちが数秒で済みます
...
次回から同じ誤認識は自動で修正されます。一度直した語と読み・綴りがよく似た
揺れ（1〜2文字違い）も同じ正解へ直します。照合するのは**自分が直したことのある語だけ**なので、
関係のない語を書き換えることはほぼありません（作者の実データ58件で誤った置換は0件）。
変な置換が起きたら、元になった学習ペアを「設定...」→「学習した修正」タブで削除してください。

入力欄への置き換えは、修正で変わった位置から後ろだけを選び直して行います。
長い文でも数秒以内に終わります。
...
| 認識 | モデル、演算精度、言語、ビームサイズ、低信頼語ハイライト、修正画面の文字サイズ、録音中の逐次認識、長い発話用のモデル |
...
| `correction_font_pt` | `14` | 修正ダイアログの本文の文字サイズ |
| `streaming_transcribe` | `false` | 録音中の逐次認識（下記参照） |
| `model_long` | `""` | 逐次認識で区切りが起きた長い発話だけに使うモデル（例: `medium`） |
...
通常は録音を止めてから認識するため、長く話すほど待ち時間も伸びます。
`streaming_transcribe` をオンにすると、発話が約25〜30秒たまるごとに次のポーズで区切って
**録音を続けながら先に認識**し、停止後は最後の区切り以降だけを認識します。

| 発話の長さ（作者のPC・`small`） | 通常 | 逐次認識 |
|---|---|---|
| 約3分 | 21〜29秒 | 5〜6.5秒 |
| 約5分 | 約52秒 | 約5秒 |

30秒未満の発話では区切りが起きないので、通常と同じ動きです。CPUは録音中に使います
（合計の計算量はほぼ同じ）。精度は通常と同等〜やや劣る程度でした。

さらに `model_long` を `medium` にすると、**区切りが起きた長い発話だけ**を高精度モデルで
認識します（短い発話は `model` のまま速く返ります）。作者の実測では長い発話の誤りがほぼ半分になり、
停止後の待ちは13〜17秒でした（`medium` を通常方式で使うと1〜2分かかります）。
`medium` は初回に約1.5GBをダウンロードし、起動後にバックグラウンドで読み込みます。
...
- 30秒を超える発話では、通常方式だと認識の後半で語彙ヒントが効きにくくなります
  （Whisperの仕様）。逐次認識をオンにすると区切った区間ごとにヒントが効きます
- 認識結果が表示されるのは録音を止めた後です（話しながら文字が出るリアルタイム表示には
  対応していません。Whisper は音声のまとまりを受け取ってから認識する方式のためです）
